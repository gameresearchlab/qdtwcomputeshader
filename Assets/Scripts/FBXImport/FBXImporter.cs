...
public class FBXImporter : MonoBehaviour{
	...
	*/
	public void ImportAllFBX()
	{
        ...
        Debug.Log(Application.dataPath + "/Models");
	...
	void BlenderObjectBuilding()
	{
		...
		while(true)
		{
			...
			case "Model:":
				Utilities.ReadSkip(fileCompl, white, ref iterator); //puts the iterator on the start of the next word
                Int32 i32 = new Int32();
                bool t = Int32.TryParse(Utilities.ReadTill(fileCompl, comma, ref iterator), out i32);
                    Debug.Log("Worked?" + t);
                impMeshes.Add(new ClassImpMesh(i32)); // next word should be modelID, with comma at end
				iterator++;//get off the comma character
				break;
